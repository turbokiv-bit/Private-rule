...
// ---- 控制中心判定: 控制中心里不动(用户只想要状态栏) ----
static BOOL drlInControlCenter(UIView* v) {
    for (UIView* cur = v; cur; cur = cur.superview) {
        ...
        if ([cn containsString:@"ControlCenter"] || [cn containsString:@"CCUI"] ||
            [cn containsString:@"CCModule"] || [cn containsString:@"CCRoot"]) return YES;
        if (cur == cur.window) break;
    }
    UIWindow* w = v.window;
    if (w) {
        NSString* wn = NSStringFromClass([w class]);
        if ([wn containsString:@"ControlCenter"] || [wn containsString:@"CCUI"]) return YES;
    }
    return NO;
}
...
static void drlDrawReadout(UIView* v, CGRect rect) {
    ...
    if (drlInControlCenter(v)) {          // 控制中心: 完全不碰
#if DRL_DIAG
        static int s_ccSkip = 0;
        if (s_ccSkip < 6) { s_ccSkip++; drlLog(@"SKIP control-center %@", NSStringFromClass([v class])); }
#endif
        return;
    }
...
static void drlUpdateExtraRing(UIView* v) {
    ...
    if (!(lvl >= UIWindowLevelStatusBar - 2.0 && lvl <= UIWindowLevelStatusBar + 2.0)) return;
    if (drlInControlCenter(v)) return;
    CGRect b = v.bounds;
