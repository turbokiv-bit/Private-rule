...
must(
    ...
    '// to the preferred group after it recovers.\n'
    '//\n'
    '// Cheap fast-path gate: if the preferred (first, fallback-order) member is\n'
    '// ALREADY the current selection, nothing needs recovering — return\n'
    '// unchanged (zero re-probe cost during steady state). Only when the\n'
    '// preferred group looks alive but is NOT selected (we are cut over to a\n'
    '// backup) do we re-probe members and ask the outer fallback Select() to\n'
    '// switch back.\n'
    ...
    '\t\treturn\n'
    '\t}\n'
    '\t// Steady-state fast path: preferred already selected -> nothing to do.\n'
    '\tmembers := s.group.loadOutbounds()\n'
    '\tif len(members) == 0 { return }\n'
    '\tpreferred := members[0]\n'
    '\tif cur := s.group.selectedOutboundTCP.Load(); cur != nil && cur == preferred {\n'
    '\t\treturn\n'
    '\t}\n'
    '\n'
    '\t// Preferred not selected -> we are cut over to a backup. Re-probe the\n'
    '\t// preferred group now (unconditionally, throttled by recoverCooldown) so\n'
    '\t// the outer URL-test history is refreshed with the CURRENT state. Do NOT\n'
    '\t// short-circuit on a stale-history check here: if the preferred group\n'
    '\t// previously went down, its history was cleared, and only an actual\n'
    '\t// re-probe (not a pre-check against that stale/empty history) can reveal\n'
    '\t// that it has recovered. A pre-re-probe groupHasLiveMember() against the\n'
    '\t// still-stale history permanently blocks recovery — avoid it.\n'
    '\ts.recoverMu.Lock()\n'
    ...
    '\ts.recoverMu.Unlock()\n'
    '\n'
    '\t// Background: re-probe every member group (refreshing the outer history),\n'
    '\t// then, if the preferred group is genuinely live AFTER this fresh probe,\n'
    '\t// ask the outer fallback Select() to switch back to it. Only re-probing a\n'
    '\t// live preferred group avoids the false-positive where a load-balance\n'
    '\t// group whose members all failed is still treated as recovered.\n'
    '\tgo func() {\n'
    ...
    '\t\t\t}\n'
    '\t\t}\n'
    '\t\t// groupHasLiveMember here runs AFTER the re-probe, so it reflects the\n'
    '\t\t// freshly updated history — this is the correct place for the live gate.\n'
    '\t\tif s.groupHasLiveMember(preferred) {\n'
