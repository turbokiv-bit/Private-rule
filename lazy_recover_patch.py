...
must(
    ...
    '\t\treturn\n'
    '\t}\n'
    '\t// Preferred not selected. Only bother if it currently looks alive at all;\n'
    '\t// if it is fully dead (no live leaf) there is nothing to switch back to yet,\n'
    '\t// so skip re-probing and wait until it recovers.\n'
    ...
    '\ts.recoverMu.Unlock()\n'
    '\n'
    '\t// Re-probe every member so the outer history is fresh (in the background),\n'
    '\t// then ask the outer fallback Select() to switch back to the preferred\n'
    '\t// group. performUpdateCheck only flips selection if Select() returns a\n'
    '\t// different (here: preferred) group, and only re-probing a live preferred\n'
    '\t// avoids the false-positive where a load-balance group whose members all\n'
    '\t// failed is still treated as recovered.\n'
    '\tgo func() {\n'
